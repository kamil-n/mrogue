...
class WeaponTemplate(ItemTemplate, type='weapon'):
    ...
    def create(self, random: bool = False, min_budget: int = 0, max_budget: int = 10) -> item.Wearable:
        if random:
            ...
            props = item.Wearable.Weapon(quality, enchantment, self.speed, self.to_hit, self.damage)
            return item.Wearable(name, self.weight, self.value, self.icon, material,
                                 quality, enchantment, self.slot, 'weapon', props)
        name = f"{self._template['material']} {self.name}"
        props = item.Wearable.Weapon(0, 0, self.speed, self.to_hit, self.damage)
        return item.Wearable(name, self.weight, self.value, self.icon, self.material,
                             0, 0, self.slot, 'weapon', props)
...
class ArmorTemplate(ItemTemplate, type='armor'):
    def __init__(self, *args):
        ...
        if len(self.keywords):
            self.material = data.materials['armor'][self.keywords[0]][self._template['material']]
        else:
    ...
    def create(self, random: bool = False, min_budget: int = 0, max_budget: int = 10) -> item.Wearable:
        if random:
            ...
            return item.Wearable(name, self.weight, self.value, self.icon, material,
                                 quality, enchantment, self.slot, 'armor', props)
        self.name = f"{self._template['material']} {self.name}"
        props = item.Wearable.Armor(0, 0, self.ac)
        return item.Wearable(self.name, self.weight, self.value, self.icon, self.material,
                             0, 0, self.slot, 'armor', props)
...
class ScrollTemplate(ItemTemplate, type='scroll'):
    ...
    def create(self, *args) -> item.Consumable:
        ...
        return item.Consumable(name, self.weight, self.value, self.icon, 1, color, self.effect, id_name, 'scroll')
...
class PotionTemplate(ItemTemplate, type='potion'):
    ...
    def create(self, *args) -> item.Consumable:
        ...
        return item.Consumable(name, self.weight, self.value, self.icon, 1, color, self.effect, id_name, 'potion')

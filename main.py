...
class Rogue:
    ...
    def __init__( self ):
        ...
        self.interface.print_at( 0, self.statusLine, 'HP:', self.interface.colors['WHITE'] );
        self.interface.print_at( 12, self.statusLine, 'AC:', self.interface.colors['WHITE'] );
        self.interface.print_at( 21, self.statusLine, 'ATK:', self.interface.colors['WHITE'] );
    ...
    def mainloop( self ):
        ...
        while key != ord( 'Q' ):
            ...
            self.level.look_around( self.player.pos, self.player.range, 'seethrough' in sys.argv );
            if not self.monsters.handle_monsters():
                ...
                win.window.addstr( 3, 2, 'You`ve defeated', self.interface.colors['WHITE'] );
                win.window.addstr( 4, 2, 'all monsters!', self.interface.colors['WHITE'] );
                win.loop();
                win.close();
            ...
            if self.player.hitPoints < 1 and 'god' not in sys.argv:
                ...
                win.window.addstr( 3, 2, 'YOU DIED', self.interface.colors['RED'] );
                win.loop();
                win.close();
                self.interface.close();
                break;
